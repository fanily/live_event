...
var getLiveArticle = function() {
	var template = MustacheTemplate('#template-article',{
...
var appendVideo = function() {
	if (config.video.playlist !== '') {
		var url = 'http://www.youtube.com/embed?autoplay=1&rel=0&loop=1&listType=playlist&list='+config.video.playlist;
	} else {
		var url = 'https://www.youtube.com/embed/'+config.video.id+'?autoplay=1&rel=0';
	}

	if (config.video.start_time !== '') {
		url += '&start='+config.video.start_time;
	}
